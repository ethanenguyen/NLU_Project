...
def get_span(label, index2tag, type = False):
    span = []
    st = -1
    en = -1
    flag = False
...
def evaluation(gold, pred, index2tag, type = False):
    """
        ...
        for j in range(len(gold_batch)):
            ...
            for item in pred_span:
                if item in gold_span:
                    right += 1
    ## total_en  = total of spans of entity in the gold data ( annotated labels )
    ## predicted = total of spans of entity found in all evaluated output
    ## right     = total of correct entity spans
    ## precision = correct / predicted correct
    ## recall    = correct / total correct
    #print( total_en, predicted, right)
    if predicted == 0:
        precision = 0
    else:
